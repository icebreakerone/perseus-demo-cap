...
app.get('/callback', async (req, res) => {
    ...
    if (!tokenEndpoint) {
        return res.status(500).send('Token endpoint is not available in the issuer metadata.');
    }
    ...
    try {
        ...
        if (!tokenResponse.ok) {
            ...
            console.error(`Error in token request: ${tokenResponse.status} ${tokenResponse.statusText}`);
        ...
        console.log('Fetching data from data server:', config.protectedResourceUrl.href);
        ...
        if (!dataResponse.ok) {
            ...
            console.error(`Error fetching data from data server: ${dataResponse.status} ${dataResponse.statusText}`);
            return res.status(500).send(`Error fetching data from data server: ${errorText}`);
        }

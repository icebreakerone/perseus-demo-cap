...
console.log(`Fetching metadata from: ${config.server.href}.well-known/oauth-authorization-server`);
const res = await customFetch(new URL('/.well-known/oauth-authorization-server', config.server).toString());

if (!res.ok) {
    console.error(`Error fetching discovery document: ${res.status} ${res.statusText}`);
...
const issuer = await client.discovery(new URL(
    '/.well-known/oauth-authorization-server', config.server),
    config.clientId,
    clientMetadata,
...
let parameters: Record<string, string> = {
    ...
    response_type: 'code',
    scope: 'https://registry.core.pilot.trust.ib1.org/scheme/perseus/license/energy-consumption-data/2024-12-05+offline_access',
    code_challenge,
...
if (!parResponse.ok) {
    console.error(`Error in PAR request: ${parResponse.status} ${parResponse.statusText}`);
